...
module.exports = BaseController.extend({
    ...
    localSignIn: async function (req, res, next) {
        if (req.user) {
            ...
            var userInfo = await UserModel.findOne({ email: req.user.email });

            if (userInfo.status != 'active') {
                req.flash('error', 'Your account does not activated, it should be activated by admin');
                return res.redirect('/auth/login'); // "5d15e8ec7fa3720920f03bd9"
            }
